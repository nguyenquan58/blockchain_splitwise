...
// =============================================================================
async function getlistIOU() {
    return BlockchainSplitwise.getlistIOU();
}
// TODO: Add any helper functions here!
async function getNeighbors(node) {
	var listIOU = await getlistIOU();
	var neighbors = [];
	for (var i=0; i<listIOU.length; i++) {
		if (listIOU[i].debtor.toLowerCase()===node.toLowerCase() && listIOU[i].amount>0) 
			neighbors.push(listIOU[i].creditor);
	}
	return neighbors;
}
...
async function getUsers() {
    ...
    var users = new Set();
    for (var i=0; i<listIOU.length; i++) {
        users.add(listIOU[i].debtor);
        users.add(listIOU[i].creditor);
    }
...
async function getTotalOwed(user) {
    ...
    var totalowed = 0;
    for (var i=0; i<listIOU.length; i++) {
        if (listIOU[i].debtor.toLowerCase()===user.toLowerCase()) 
            totalowed += listIOU[i].amount;
    }
    return totalowed;
}
	  ...
	  for (var j = 0; j < txns.length; j++) {
	  	...
	  	if (txn.to.toLowerCase() === contractAddress.toLowerCase()) {
			...
			if (func_call && func_call.name === "add_IOU") {
				...
				if (txn.from.toLowerCase()===user.toLowerCase() || args[0].toLowerCase()===user.toLowerCase()) {
...
async function add_IOU(creditor, amount) {
	...
	if (path!=null) {
		...
		for(var i=0; i<path.length-1; i++) {
			min = await BlockchainSplitwise.lookup(path[i], path[i+1]);
			smallest = Math.min(smallest, min);
		}
		console.log(smallest);
		for (var i=0; i<path.length-1; i++) {
			res = await BlockchainSplitwise.connect(provider.getSigner(path[i])).add_IOU(path[i+1], smallest, false);
		}
	}
	res = await BlockchainSplitwise.connect(provider.getSigner(defaultAccount)).add_IOU(creditor, amount-smallest, true);
